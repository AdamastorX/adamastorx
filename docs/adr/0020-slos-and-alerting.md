...
| `aggregator` (backlog #81, added by backlog #90/ADR 0031) | non-5xx rate on `GET /aggregates`/`GET /aggregates/{ticker}` (`http_server_requests_seconds_count{uri=~"/aggregates.*"}`, excluding `/actuator/*` same as every other service's own row) — a different question from the market pipeline's own end-to-end freshness row below, not this row's job | p95 via `http_server_requests_seconds_bucket`; state-restore duration (`aggregator_state_restore_duration_seconds`, already real, the metric #81's own AC required) is the saturation-adjacent signal for the ~45s restart window, not a latency SLI in the request sense |
| **market pipeline (end-to-end)** (backlog #91) | **Freshness, not availability** — the review's own named "one SLO a streaming pipeline actually needs": `aggregator_price_freshness_seconds` (real Finnhub trade timestamp → `aggregator`'s own topology processing that record), measured only against `source="WEBSOCKET"` ticks. `market-data-ingestor`'s `FinnhubQuotePoller` REST-poll fallback publishes onto the same topic in the same shape every 30 minutes, by design, to cover off-hours gaps (`source="POLL_FALLBACK"`) — a real trap found during roadmap review before this row existed: an SLI blending both sources would show a real, correct 30-minute lag on every fallback tick, either alerting constantly or going meaningless during real market hours. `AggregatorPriceFreshnessSlow` filters to the websocket series by construction (the metric's own label), not a threshold exclusion | p95 threshold, 30s — a deliberately simple v1 pick (this pipeline's own websocket-to-processed path should sit at a handful of seconds end to end on a healthy pipeline; not derived from a real historical distribution this cluster doesn't have yet, same as every other threshold in this table), not a distinct latency SLI from the availability column — freshness *is* this row's latency question |
