...
- Dependencies: none.
- Priority: P1. Labels: `platform`. **Decision made and real accounting done; PR open, deliberately not yet merged (platform#81)**: `kubectl describe node` confirmed the baseline live — `cpu 3995m (99%)` of 4000m allocatable requested, against real node CPU *usage* of only 1.7-1.8 cores (42-45%, `kubectl top node`), the same request-vs-usage gap the item's own purpose section predicted. A per-workload `kubectl top pod -A` breakdown, sampled three times over ~6 minutes (not a single instant), compared against each component's committed `resources.requests.cpu` in `platform`, found the gap concentrated in exactly the kind of components this item called out — chart defaults and early estimates never revisited once real traffic existed, not a general problem across every component: the three Bitnami Postgres instances and Redis (`resourcesPreset: small`, 500m each, real usage 9-45m) and `watchlist-service`/`clinvar-service` (250m each, real usage 3-32m). Decision taken: trim CPU *requests* only on those six, to 100-150m (3-8x real observed usage each), leaving CPU *limits* and all memory values untouched everywhere — memory already has confirmed real slack, and this item's own root cause is a request problem, not a usage or ceiling problem. `api`/`workers` requests deliberately left alone (backlog #35's CrashLoopBackOff was a JVM cold-start CPU story; trimming steady-state waste there would risk repeating it in reverse) and Kafka's CPU left alone too (matching #75's own precedent of using memory, not CPU, as Kafka's safe lever). Calculated effect of the diff: total requested CPU would drop from 3995m to 2145m, 99%→~54% of allocatable — enough for KEDA's #63 finding (`workers` needing 3× 250m = 750m) to fit with real room to spare — but this is a calculation from the committed diff, not yet a live-measured number: this project's own standing rule is that a live-only `kubectl` edit gets reverted by selfHeal almost instantly, so every real change here has to go through a real PR, merge, and ArgoCD sync before it counts as proven, and per this session's own instruction the PR (`platform#81`, all CI checks green: `terraform`, `manifests`, `resource-limits`, `shell`, `helm-render`, `ci`) is left open for human review rather than self-merged. The live before/after `kubectl describe node` comparison and per-pod health check remain open until that merge happens — tracked here rather than claimed early.
