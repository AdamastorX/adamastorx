...
**23. Chaos / failure-injection test plan — 2/3 done**
...
- Dependencies: #45 (a stream processor over near-zero traffic aggregates nothing), #23 scenario 3.
- Priority: P2. Labels: `backend`, `observability`, `architecture`.

**56. Per-tenant API keys and rate limiting at the edge (Traefik middleware, not a new service)**
- Purpose: ADR 0021 removed `gateway` and stated the condition for revisiting an edge layer precisely: "if a real cross-cutting concern (auth, rate-limiting across several backends) ever arrives, reintroducing an edge service — or using Traefik middleware — is a deliberate future decision." With `clinvar-viewer` calling `api` from a browser, #53's subscriber-facing endpoints, and #45 generating continuous load, that concern is real: there is currently no authentication of any kind on any public path, no per-caller quota, and no way to tell one caller's traffic from another's. **This deliberately does not resurrect `gateway`** (ADR 0022) — `gateway` was cut for having no function, and an empty forwarder is not restored by giving it a job it never had. The cross-cutting concern is implemented where it belongs at this scale: Traefik middleware, which is already the ingress every service sits behind.
- Acceptance Criteria: API-key (or equivalent) authentication enforced via Traefik middleware on the public paths, with keys managed as real Kubernetes Secrets (and reconciled with whatever #36 settles on for secret provisioning, not a second competing mechanism). Per-key rate limiting with a documented limit and a `429` response proven live. `clinvar-viewer` updated to authenticate, and #45's generator issued its own key so synthetic traffic is attributable at the edge as well as in metrics. Rate-limit rejections and per-key request rates visible in Prometheus and on a dashboard. A short written note (not necessarily an ADR) recording that this satisfies ADR 0021's stated reintroduction condition via middleware rather than a service, so the decision trail stays intact.
- Dependencies: #45, #53.
