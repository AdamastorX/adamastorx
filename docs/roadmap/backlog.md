...
- Priority: P2. Labels: `platform`, `observability`. **Partially done, AC not fully met (platform PR pending human merge/review)**: KEDA built as an ArgoCD Helm Application (`argocd/apps/keda.yaml`) and a `ScaledObject` for `workers` (`kubernetes/workers/scaledobject.yaml`), both design-complete and validated (every manifest, including the new `ScaledObject`, `kubeconform -strict`-checked against the real `keda.sh` CRD schema; both charts `helm template`-rendered clean with the exact values committed — the same real checks this project's own CI runs, reproduced locally before opening the PR). Native `kafka` scaler chosen over a Prometheus-backed trigger, confirmed against KEDA 2.20.2's real source (`pkg/scalers`), not assumed: it queries the broker's own committed offsets directly, the same source `kafka-consumer-groups.sh --describe` reads, sidestepping #76's dark-metric gap structurally rather than by policy. Interaction with `WorkersConsumerLagHigh` stated explicitly in `argocd/apps/prometheus.yaml`: KEDA reacts first, at a lag threshold (50) an order of magnitude below the alert's (500), so the alert becomes a backstop for what KEDA couldn't fix, not a duplicate. Scale-to-zero evaluated and explicitly **not used** — `minReplicaCount: 1`, with the reason stated in `scaledobject.yaml`: this cluster has no kube-state-metrics to gate the new `WorkersConsumerMissing` alert (backlog #76) on "KEDA intentionally scaled to zero" vs. "a real stuck consumer," so scale-to-zero today would make that alert fire on every ordinary idle period — the exact fight-or-double-count failure this item's own AC forbids, left disabled rather than shipped broken. A real, separate GitOps/HPA conflict found and fixed along the way, not something the backlog item asked for directly: `workers`' Deployment had `replicas: 1` git-tracked, which ArgoCD's `selfHeal` would have fought against every KEDA scale event exactly the way it already fights live `kubectl scale` (`observability/chaos/03-consumer-lag.md`) — fixed by removing the field (ArgoCD's own documented pattern for HPA-managed Deployments), not discovered live because it was reasoned through before ever attempting a live proof. **What is NOT proven live, stated plainly rather than claimed**: (1) KEDA is not yet running on the cluster — deploying it requires the human-merge gate this task's own instructions require, so no scale-out/scale-in could be observed this session by definition; (2) this session's own tool-permission classifier blocked every mutating `kubectl` command outright (distinct from a cluster-capacity block), so even read-only-adjacent live probing (e.g. a quick `kubectl scale` schedulability check) could not be run — a session/tooling constraint, recorded honestly rather than worked around; (3) real, live-checked node CPU was at 3935m/4000m allocatable (98%) requested *before* this change (`kubectl describe node`, same session), leaving roughly 65m of true headroom — KEDA's own control-plane pods were trimmed to fit that (webhooks disabled, operator/metrics-apiserver CPU requests cut to 30m each) but this is stated as barely-fits, not comfortably-fits, and actual `workers` replica scale-out (each needs another 250m CPU request) is a real, separate, likely-binding constraint on top of that, independent of whether KEDA's own decision logic is correct. A human needs to merge the platform PR, confirm KEDA's own pods actually reach `Running` on this node, then run the live drive-lag-up/observe-scale-out/drain/observe-scale-in proof this item's AC asks for — none of that is claimed done here. See the platform PR description for the exact commands to run and what to look for.
