...
- Priority: P2. Labels: `observability`. **In progress, real work landed but not yet closeable (platform PR open, not merged)** — Pyroscope re-verified as still the right real-world fit (bundled Grafana datasource since 12.3+, confirmed against this cluster's 12.8.0 chart) but the AC's suggested instrumentation path (an in-app SDK baked into a rebuilt image) was found to have a real, same-session-blocking dependency: it needs a services-repo merge + a CI-published GHCR image, and this project's own review discipline is PR-then-human-merge, not self-merge. ADR 0028 records the real alternative considered and rejected (Grafana Alloy's external-attach `pyroscope.java`/`pyroscope.ebpf`, already prototyped working in River config, but needing the Alloy DaemonSet to run privileged/root/hostPID cluster-wide — too large an escalation to take unilaterally inside this item) and the middle path actually shipped: the real Pyroscope Java/Python SDKs, injected via an unprivileged init-container (a shared `emptyDir` + an ordinary non-root container fetching the real published agent/package) instead of a rebuilt image. **What's genuinely verified this session**: every manifest passes `kubectl apply --dry-run=server` against this cluster's real API server, and the Pyroscope chart (2.2.0, pinned, `deprecated: true` checked and clear) renders cleanly via a real local `helm template` against the pinned version — one StatefulSet, a real `pyroscope.pyroscope.svc.cluster.local:4040` Service, the configured 3Gi PVC and 150m/500m CPU + 256Mi/768Mi memory resources all confirmed present in the rendered output, not assumed from the values file alone. Sizing is real: this cluster had `cpu 2145m/4000m (53%)` requested (`kubectl describe node`, 2026-08-01, post-#77) and 169GB free disk (`df -h /`) before this item, so the added ~200m CPU / ~380Mi memory request and 3Gi nominal PVC fit with real room to spare, following #77's own "measured, not assumed" discipline. **What's not yet proven live**: this session's own tooling blocked every cluster-mutating command (`kubectl apply`/`patch` against the real cluster), a hard session constraint stated honestly rather than worked around — so nothing was actually deployed, no real flame graph of the #35 reproduction was captured, `clinvar-service`'s VCF-scan profile was not taken, and real PVC growth-vs-72h-retention numbers (`local-path`'s unenforced quota, #21d, already accounted for as nominal-only) are not yet measured. The platform PR description has the exact commands for a human to run post-merge to close all four.
