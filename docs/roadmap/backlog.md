...
- Dependencies: #21.
- Priority: P1. Labels: `observability`, `platform`. **Unblocked by #92 (ADR 0031, 2026-08-06)**: this item's "whichever metric is already scraped" caveat was the real blocker — no node filesystem metric is scraped at all while kube-state-metrics/node-exporter stay disabled. #92 enables them; this alert is then written against the real node metric, not a proxy.
...
- Dependencies: #48 (multi-node substrate, M7).
...
- Priority: P1. Labels: `backend`, `observability`, `architecture`.
...
- Priority: P0 (Phase 1). Labels: `platform`, `documentation`. **Descoped by the owner (2026-08-06)** — staying on the existing `*.local.adamastorx.test` local-access setup for now rather than standing up a Cloudflare Tunnel; a deliberate, explicit deferral, not silently dropped. Revisit when public reach is actually wanted; the phasing/risk-split reasoning above still applies whenever this is picked back up.
...
- Dependencies: none.
- Priority: P0. Labels: `platform`, `architecture`.
...
- Dependencies: none.
- Priority: P1. Labels: `documentation`. **Recurred a third time, 2026-08-06** (found by the staff-engineer review, `docs/reviews/`): overview.md again claimed M13's five services "do not exist yet" two days after this same repo's backlog declared M13 Done — the exact drift this item's AC names, on the exact file it names, after its fix had shipped. Fixed again in the ADR 0031 PR, but two process failures on one file is the evidence: **#97 replaces the checklist approach with a CI roster check**, and this item closes as superseded-by-automation once #97 lands rather than being marked Done a second time and left to recur a fourth.
...
- Dependencies: none.
- Priority: P1. Labels: `platform`, `observability`, `bug`. **Partially done (2026-08-05, services#58, platform#106)** — (a) fixed and deployed live. Grepped the whole module for the bare `.toString()`/`.getMessage()` exception-logging pattern rather than trusting memory of where the one known gap was: found and fixed four sites, not just `onError` — `FinnhubWebSocketClient`'s connect-failure and message-parse-failure logs, and `StockPriceTickPublisher`'s publish-failure log, alongside the one already named. All now pass the real `Throwable` via SLF4J's `log.warn(msg, ex)` overload. `FinnhubQuotePoller`'s `ex.getClass().getSimpleName()`-only logging (services#55) was deliberately left untouched — that one exists specifically to avoid leaking the real Finnhub API key, which rides in the request URI some exception messages embed; a real, different tradeoff, not an oversight. Deployed live: fresh headroom check (unchanged request, same-request redeploy), new pod reached `1/1 Running` (a real 37.9s JVM cold start, no crash), logs confirmed a real Finnhub reconnect and re-subscribe to all 5 tickers post-deploy — no regression. (b) and (c) remain open: no recurrence has happened since to investigate, and the liveness-probe question is now moot in the exact sense #85(b) closed it — a permanently poisoned producer that still passes `KafkaStreams`-shaped health checks isn't this service's failure mode (no Kafka Streams here, plain producer/websocket), so (c) needs its own, separate answer if ever revisited, not an assumption that #85(b)'s fix already covers it.
...
- Dependencies: none.
- Priority: P2. Labels: `backend`, `observability`. **Companion item #105 (the HikariCP/Postgres side) added by ADR 0031** — SESSION_STATE recorded both blocking-call findings but only this one got an item. Both should state explicitly whether #60 (Istio timeouts/circuit-breaking, M7) is their intended answer; if so, these two items' documented hangs become #60's before/after evidence rather than independent fixes.
