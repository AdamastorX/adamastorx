...
- Dependencies: #81.
- Priority: P1. Labels: `platform`, `observability`, `bug`. **Partially done (2026-08-03, services#52/#53, platform#98/#99)** — (a) fixed, verified live, twice over. `services#52` added `libstdc++`/`libgcc`, resolving the RocksDB/Alpine `UnsatisfiedLinkError`; redeploying with that alone surfaced a **second, independent real bug** in the exact same silent-but-Healthy shape: `BoundedRocksDbConfigSetter` constructed a fresh `BlockBasedTableConfig` instead of reusing the instance Kafka Streams itself attaches to `Options`, breaking its internal RocksDB metrics-recorder wiring the moment a store was actually opened against real traffic (`org.apache.kafka.streams.errors.ProcessorStateException`, again `SHUTDOWN_CLIENT`, again a `Healthy`-looking pod). `services#53` fixed that too (retrieve-and-mutate `options.tableFormatConfig()`, exactly what the real exception message names as the fix). Redeployed a third time: real logs confirmed `RUNNING` sustained for 60+ real seconds with no further exception, `GET /aggregates` correctly returned `200 []`/`404` (real "no data yet" responses, not `503`) — the app is genuinely healthy now, not just Kubernetes-reported healthy. Real trade-tick traffic wasn't flowing at verification time (outside real US market hours, same honest gap #78 already recorded), so a fully-populated `GET /aggregates/{ticker}` response with real data is still unconfirmed — a real, stated follow-up, not a blocker. (b) **still open**: the liveness-probe gap itself (readiness/liveness not gating on Kafka Streams state) was not changed — both real bugs found this session happened to be caught by a human actively watching logs after each sync, not by the probe; that gap is real and remains, tracked here rather than silently closed alongside (a).
